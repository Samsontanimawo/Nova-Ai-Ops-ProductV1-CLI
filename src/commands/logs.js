  ...
  cmd
    ...
    .action(async (query, opts) => {
      try {
        ...
        const data = await api.get(`/logs/explorer/logs?${params}`);
        ...
        console.log(createTable(
          ...
          logs.map(l => {
            ...
            return [
            timeAgo(l.ts || l.timestamp || l.time),
            lvl === 'error' ? chalk.red(l.level) : lvl === 'warn' ? chalk.yellow(l.level) : chalk.gray(l.level || '-'),
    ...
    .action(async (opts) => {
      try {
        ...
        const poll = async () => {
          ...
          if (opts.level) params.set('level', opts.level);
          const data = await api.get(`/logs/explorer/logs?${params}`);
          const logs = Array.isArray(data) ? data : (data.data || data.logs || data.entries || []);
          for (const l of logs) {
            const lvl = (l.level || 'info').toUpperCase().padEnd(5);
            const lvlLower = lvl.trim().toLowerCase();
            const color = lvlLower === 'error' ? chalk.red : lvlLower === 'warn' ? chalk.yellow : chalk.gray;
            console.log(`  ${chalk.gray(new Date(l.ts || l.timestamp || Date.now()).toLocaleTimeString())} ${color(lvl)} ${chalk.cyan(l.service || '-')} ${l.message || ''}`);
          }
        };
        await poll();
